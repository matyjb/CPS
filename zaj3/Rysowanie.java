...
class Ramka extends JFrame {
    ...
    public void UstawMnieNaŚrodku() {
        ...
        /*
         * Toolkit.getDefaultToolkit to metoda statyczna, zwracająca obiekt, który
         * opisuje środowisko graficzne "zewnętrzne" w stosunku do okna. Do informacji
         * podstawowych należy oczywiście rozmiar ekranu lub pulpitu wirtualnego, zawsze
         * wyrażający się w pikselach. Znając rozmiar okna oraz rozmiar ekranu, w prosty
         * sposób możemy umieścić okno na środku pulpitu.
         */
...
/*
 * Klasa dziedzicząca z klasy JPanel W klasie tej przeciążeniu poddano metodę
 * paintComponent, używaną jak nazwa wskazuje, do rysowania. Rysowanie może
 * odbywać się tylko za pomocą tej metody, a dodatkowo metoda ta jest wywoływana
 * tylko wtedy, gdy istnieje potrzeba przerysowania okna. Jest to bardzo istotna
 * informacja, bo rysunek w rzeczywistości powinien być w pewien sposób
 * buforowany, gdyż w innym przypadku zotanie on zamazany czym innym, na
 * przykład innym oknem.
 */
class PanelOkna extends JPanel {
    ...
    private void RysujWidmo(Graphics NaTymRysujemy) {
        ...
        for (i = 0; i < WidmoDoRysowania.length - 1; i++) {
            ...
            y1 = PrzeliczWspółrzędną(MaxY, min_wartość_widma, max_wartość_widma,
                    WidmoDoRysowania[i].getWidmoAmplitudowe());
            y1 = Lustro(MaxY, y1);

            x2 = PrzeliczWspółrzędną(MaxX, f_min, f_max, WidmoDoRysowania[i + 1].getCzęstotliwość());
            y2 = PrzeliczWspółrzędną(MaxY, min_wartość_widma, max_wartość_widma,
                    WidmoDoRysowania[i + 1].getWidmoAmplitudowe());
            y2 = Lustro(MaxY, y2);
